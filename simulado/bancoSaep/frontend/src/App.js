...
function App() {
  ...
  const handleLoginSuccess = async (userData) => {
    try {
      const response = await axios.get(
        `http://localhost:3001/api/inscricoes/usuario/${userData.id}`
      );

      const qtdInscricoes = response.data.qtdInscricoes;
      ...
      console.error("Erro ao buscar número de inscrições:", error);
      setUser(userData); // Define o usuário mesmo que a requisição falhe
    }
  };
  ...
  const handleInscricao = (cursoId) => {
    if (!user || !user.id) {
      console.error(
        "Usuário não está logado ou ID do usuário está indefinido."
      );
    ...
    axios
      .post("http://localhost:3001/api/inscricao", {
        usuarioId: user.id,
        cursoId: cursoId,
      })
      .then(() => {
          ...
          prevCursos.map((curso) =>
            curso.id === cursoId
              ? {
                  ...curso,
                  isInscrito: true,
                  qtdInscricoes: curso.qtdInscricoes + 1,
                }
              : curso
          )
        );
  ...
  useEffect(() => {
    if (user) {
      axios
        .get(`http://localhost:3001/api/cursos?usuarioId=${user.id}`)
        .then((response) => setCursos(response.data))
        .catch((error) => console.error("Erro ao carregar cursos:", error));
    }
  }, [user, cursoSelecionado, isComentarioslOpen]);
    ...
    <div className="App">
      ...
      <div className="container">
        <Perfil
          ...
          perfilInsc={user && user.qtdInscricoes ? user.qtdInscricoes : "0"}
